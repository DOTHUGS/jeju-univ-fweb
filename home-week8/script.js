...
function getBestScore() {
  return localStorage.getItem('bestScore') || '기록 없음';
}
...
typedValueElement.addEventListener('input', () => {
    ...
    if (typedValue === currentWord && wordIndex === words.length - 1) { // 마지막 단어까지 정확히 입력했는 지 체크
      ...
      const finalTime = (elapsedTime / 1000).toFixed(2); // 시간을 초 단위로 변환, 소수점 2자리로 표시
      const message = `CONGRATULATIONS! You finished in ${finalTime} seconds.` ;  // 타이핑 완료 메시지
      messageElement.innerText = message; //생성된 메시지 화면에 표시
      typedValueElement.disabled = true;
      startButton.disabled = false;

      // 최고 점수 저장 및 표시
      saveBestScore(finalTime);
      const bestScore = getBestScore();

      // 모달 창에 결과 표시
      finalMessageElement.innerText = `타이핑 완료 시간: ${finalTime} 초 \n오타수 : ${mistakes}`;
      bestScoreElement.innerText = `최고 기록: ${bestScore} 초`;
      resultModal.style.display = 'block'; // 모달 창 표시
